...
# output_file = 'task_LCTW_MRC.json'
...
# File paths
data_file = "task_LCTW_MRC.json"
output_results_file = "LCTW-doubao_outputs.json"
eval_results_file = "LCTW-doubao_results.json"
...
#         messages = [
#             {"role": "system", "content": "You are an assistant who is good at text reading comprehension. You only need to output brief answers without any extra explanations."},
#             {"role": "user", "content": f"{instruction}\n\n{input_text}"}
#         ]
#         try:
...
instruction_en = ("Translate the following sentences from English to Korean. "
                 ...
                 "The context contains a set of translations from English to Korean. "
                 "Make full reference to the context to better understand how to translate from English to Korean, "
...
for i, item in enumerate(tqdm(dataset, desc="🚀 处理数据", unit="条")):
    instruction="Please complete the reading comprehension task based on the following text, and only need to output concise answers to the questions at the end."
    ...
    history = [[input_en, input_text]]
    model_output = translate(system_content, instruction_en, output_en, history)
...
#
# # 文件路径
# input_file = "LCTW-gpt-4o_outputs.json"
# output_file = "LCTW-gpt-4o_results.json"
#
...
#
# # 提取 em 和 f1 分数
# eval_scores = [{"em": item["em"], "f1": item["f1"]} for item in data if item["output_zh"] != "ERROR"]
#
# # 计算数据量和均值
# num_samples = len(eval_scores)
# avg_scores = {
